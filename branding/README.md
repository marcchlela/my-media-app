...
Run `powershell -ExecutionPolicy Bypass -File branding/export-icons.ps1` from the repository root to regenerate PNG exports. The script uses an installed Microsoft Edge browser to render each complete SVG into one 1024px master, then Windows' built-in high-quality image renderer scales that master to the smaller web sizes. This keeps the full centered artwork at every size without depending on Edge's minimum viewport behavior.
