...
function render() {
  const featured = nowShowing[1];
  document.getElementById('app').innerHTML = `
    <div class="app electric">
      <div class="ambient-room"></div>
      ${sidebar(`<div class="lounge-prop"><div class="lamp"></div><div class="chair"></div></div>`)}
      <main class="main">
        ${topbar()}
        <section class="hero lounge-wall">
          <div class="ticket-feature">
            <span class="ticket-label">Featured screening</span>
            <h1>${featured.fullTitle}</h1>
            <p class="feature-meta">Movie · Drama · 1h 48m · ${featured.screen}</p>
            <p class="feature-desc">A quiet night in your private cinema lobby. Continue where you left off, with your library presented like a real theater wall.</p>
            <div class="feature-actions"><button>▶ Play Now</button><button class="circle">+</button></div>
          </div>
          <div class="poster-wall">
            ${nowShowing.map(item => posterCard(item, 'lit-frame')).join('')}
          </div>
          <div class="wall-sconce"></div>
        </section>
        <section class="continue-section">
          <div class="section-head"><h2>Continue Watching</h2><a>See all</a></div>
